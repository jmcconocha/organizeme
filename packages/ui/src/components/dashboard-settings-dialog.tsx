...
import type { ViewMode, TableColumn, CardDensity } from "@organizeme/shared/types/dashboard-settings"
...
export function DashboardSettingsDialog({ open, onOpenChange }: DashboardSettingsDialogProps) {
  ...
  // Load app settings when dialog opens
  React.useEffect(() => {
    if (open && mounted) {
      dataProvider.getAppSettings().then((appSettings) => {
        setProjectsPath(appSettings.projectsPath)
        setSavedPath(appSettings.projectsPath)
        setPathSaved(false)
      })
    }
  }, [open, mounted, dataProvider])

  const handleSaveProjectsPath = async () => {
    setPathSaving(true)
    try {
      const updated = await dataProvider.updateAppSettings({ projectsPath })
      setSavedPath(updated.projectsPath)
      setPathSaved(true)
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <div>
              <h3 className="text-sm font-semibold mb-1">Projects Folder</h3>
              <p className="text-sm text-muted-foreground">
                Set the directory to scan for projects. Leave empty to use the default.
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={pathSaving || projectsPath === savedPath}
                className="shrink-0"
              >
                {pathSaved ? (
